...
def compute_loss(p, targets, model):  # predictions, targets, model
    device = p[0].device
    lcls = torch.tensor([0.], dtype=torch.float32, device=device)
    lbox = torch.tensor([0.], dtype=torch.float32, device=device)
    lobj = torch.tensor([0.], dtype=torch.float32, device=device)
